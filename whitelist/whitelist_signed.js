...
const ICO_ABI = require('../build/contracts/MenloTokenPresale.json').abi;
...
filterAddresses(ARRAY_OF_ADDRESSES).then(async (toWhitelist) => {
    let currentInvestors = await myContract.methods.whitelistAddresses(toWhitelist, true).call();
...
async function sendTransactionToContribution({array, slice, addPerTx, nonce}) {
    ...
    let encodedData = myContract.methods.whitelistAddresses(arrayToProcess, true).encodeABI();
